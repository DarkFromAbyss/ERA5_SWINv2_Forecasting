...
# Đường dẫn tới file config.yaml từ thư mục gốc của dự án
...
def save_config(config_data):
    """Ghi dữ liệu cấu hình vào file config.yaml kèm ghi chú phân đoạn cho đẹp."""
    try:
        with open(CONFIG_PATH, "w", encoding="utf-8") as f:
            # Sử dụng default_flow_style=False để ghi file dạng block dễ đọc
...
def main():
    st.set_page_config(
        page_title="Thiết lập cấu hình mô hình Swin V2 - ERA5",
        layout="wide",
        page_icon="⚙️",
    )

    st.title("⚙️ Bảng Điều Khiển Cấu Hình Hệ Thống (config.yaml)")
    st.markdown(
        "Thay đổi các tham số dưới đây để cập nhật trực tiếp cấu hình huấn luyện và giao diện dự báo thời tiết."
    )
    ...
    # Tạo form để gom cụm nút Submit (tránh việc giao diện reload liên tục khi gõ chữ)
